...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const byClient = new Map<ClientKey, { name: string; nif: string; count: number; total: number; paid: number; outstanding: number }>();
    for (const d of docs) {
      const k = d.buyer?.nif || d.buyer?.name || '—';
      const rec = byClient.get(k) || { name: d.buyer?.name || '—', nif: d.buyer?.nif || '—', count: 0, total: 0, paid: 0, outstanding: 0 };
      const paid = typeof d.payment?.paidAmount === 'number' ? d.payment!.paidAmount! : (d.payment?.status === 'paid' ? (d.totals?.total || 0) : 0);
      const total = d.totals?.total || 0;
      rec.count += 1;
      rec.total = round(rec.total + total);
      rec.paid = round(rec.paid + paid);
      rec.outstanding = round(rec.outstanding + Math.max(total - paid, 0));
      byClient.set(k, rec);
    }

    const clients = clientStore.getAllClients();
    for (const c of clients) {
      const k = c.nif || c.name;
      if (!byClient.has(k)) {
        byClient.set(k, { name: c.name, nif: c.nif || '—', count: 0, total: 0, paid: 0, outstanding: 0 });
      }
    }

    const rows = Array.from(byClient.values()).sort((a,b) => b.total - a.total).slice(0, 100);
    ...
    const drawHeader = (doc: jsPDF) => {
      ...
      doc.setFont('helvetica', 'bold'); doc.setFontSize(13); doc.text('Relatório — Conta Corrente de Clientes (Gestão de Clientes)', pageWidth - 12, 16, { align: 'right' });
    ...
    const info = [
      ...
      `Em dívida: ${formatCurrency(totals.outstanding)} AOA`,
    ];
    let y = 41; for (const t of info) { pdf.text(t, 12, y); y += 5; }

    autoTable(pdf, {
      startY: y + 6,
      head: [["Cliente", "NIF", "Docs", "Total", "Pago", "Em Dívida"]],
      body: rows.map(r => [ r.name, r.nif, String(r.count), formatCurrency(r.total), formatCurrency(r.paid), formatCurrency(r.outstanding) ]),
    ...
    res.setHeader('Content-Disposition', 'inline; filename="relatorio-conta-corrente-clientes.pdf"');
    res.setHeader('Content-Length', buffer.length);
    res.send(buffer);
  } catch (e) {
    console.error('clients current account report error', e);
