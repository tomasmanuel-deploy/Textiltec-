...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const docs = documentStore.getAllDocuments().filter(d => {
      ...
      const pending = d.payment?.status !== 'paid';
    ...
    const rows = docs.map(d => {
      const paid = typeof d.payment?.paidAmount === 'number' ? d.payment!.paidAmount! : 0;
      const outstanding = round((d.totals?.total || 0) - paid);
      return {
        seriesNum: `${d.series}-${String(d.sequentialNumber).padStart(4, '0')}`,
        date: format(new Date(d.issueDate), 'dd/MM/yyyy', { locale: pt }),
        client: d.buyer?.name || '',
        nif: d.buyer?.nif || '',
        total: d.totals?.total || 0,
        paid,
        outstanding,
        due: d.payment?.dueDate ? format(new Date(d.payment.dueDate), 'dd/MM/yyyy', { locale: pt }) : '—'
      };
    ...
    const drawHeader = (doc: jsPDF) => {
      ...
      doc.setFont('helvetica', 'bold'); doc.setFontSize(13); doc.text('Relatório — Clientes Devedores (Em Dívidas)', pageWidth - 12, 16, { align: 'right' });
    ...
    autoTable(pdf, {
      startY: y + 6,
      head: [["Série/Nº", "Data", "Cliente", "NIF", "Total", "Pago", "Em Dívida", "Vencimento"]],
      body: rows.map(r => [ r.seriesNum, r.date, r.client, r.nif, formatCurrency(r.total), formatCurrency(r.paid), formatCurrency(r.outstanding), r.due ]),
    ...
    res.setHeader('Content-Disposition', 'inline; filename="relatorio-clientes-devedores.pdf"');
