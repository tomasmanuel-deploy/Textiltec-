...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const docs = scopedDocs.filter(d => {
      ...
      return (!start || paidDate >= start) && (!end || paidDate <= end) && d.payment?.method === 'bank_transfer';
    });
    ...
    const drawHeader = (doc: jsPDF) => {
      ...
      doc.setFont('helvetica', 'bold'); doc.setFontSize(13); doc.text('Relatório — Fluxo de Contabancaria (Movimentos Conta)', pageWidth - 12, 16, { align: 'right' });
    ...
    pdf.setFont('helvetica', 'normal'); pdf.setFontSize(9); pdf.text(`Total recebimentos (bancário): ${formatCurrency(total)} AOA`, 12, 41);
    ...
    res.setHeader('Content-Disposition', 'inline; filename="relatorio-fluxo-banco.pdf"');
    res.setHeader('Content-Length', buffer.length);
    res.send(buffer);
  } catch (e) {
    console.error('bank-flow report error', e);
